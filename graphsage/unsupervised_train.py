...
#core params..
flags.DEFINE_string('model', default='graphsage_mean', help='model names. See README for possible values.')
...
def construct_placeholders():
    # Define placeholders
    placeholders = {
        'batch1' : tf.placeholder(tf.int32, shape=(None), name='batch1'), # left_node
        'batch2' : tf.placeholder(tf.int32, shape=(None), name='batch2'), # right_node, 也是label [batch_size, 1]
...
def train(train_data, test_data=None):
    G = train_data[0] # edge由两个node定义。adj_matrix的结构是个map，key是node，value是node_list
    features = train_data[1] # [14756,50] 每个node有50维特征
    id_map = train_data[2]
    ...
    # 9716 train nodes
    # 5039 test nodes
    adj_info_ph = tf.placeholder(tf.int32, shape=minibatch.adj.shape) # [14756,100] 一个node有100个neighbor，neighbor_id可重复
    adj_info = tf.Variable(adj_info_ph, trainable=False, name="adj_info") # [14756,100]

    if FLAGS.model == 'graphsage_mean':
        # Create model
        sampler = UniformNeighborSampler(adj_info)
        layer_infos = [SAGEInfo("node", sampler, FLAGS.samples_1, FLAGS.dim_1),  # samples_1=25, dim_1=128
                            SAGEInfo("node", sampler, FLAGS.samples_2, FLAGS.dim_2)] # samples_2=10, dim_2=128
