...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx, 
        ...
        self.G = G
        self.nodes = G.nodes() # list，14755个node
        self.id2idx = id2idx
        self.placeholders = placeholders
        self.batch_size = batch_size # 512
        self.max_degree = max_degree # 100
        self.batch_num = 0

        self.nodes = np.random.permutation(G.nodes())
        self.adj, self.deg = self.construct_adj() # deg是neighbors的set的大小
        ...
        if not n2v_retrain:
            self.train_edges = self._remove_isolated(self.train_edges) # [1895817,2]
            self.val_edges = [e for e in G.edges() if G[e[0]][e[1]]['train_removed']] # [75407,2]
        else:
            if fixed_n2v:
    ...
    def construct_adj(self):
        ...
        deg = np.zeros((len(self.id2idx),)) #[14755]

        for nodeid in self.G.nodes():
            if self.G.node[nodeid]['test'] or self.G.node[nodeid]['val']:
                continue
            neighbors_list = np.array([self.id2idx[neighbor]
                for neighbor in self.G.neighbors(nodeid)
                if (not self.G[nodeid][neighbor]['train_removed'])]) # [neighbor_num]
            deg[self.id2idx[nodeid]] = len(neighbors_list)
            if len(neighbors_list) == 0:
                continue
            if len(neighbors_list) > self.max_degree:
                neighbors_list = np.random.choice(neighbors_list, self.max_degree, replace=False)
            elif len(neighbors_list) < self.max_degree:
                neighbors_list = np.random.choice(neighbors_list, self.max_degree, replace=True) # [100] 有重复
            adj[self.id2idx[nodeid], :] = neighbors_list
        return adj, deg

    def construct_test_adj(self):
        adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree)) # [14756,100]
        for nodeid in self.G.nodes():
            neighbor_list = np.array([self.id2idx[neighbor]
                for neighbor in self.G.neighbors(nodeid)])
            if len(neighbor_list) == 0:
                continue
            if len(neighbor_list) > self.max_degree:
                neighbor_list = np.random.choice(neighbor_list, self.max_degree, replace=False)
            elif len(neighbor_list) < self.max_degree:
                neighbor_list = np.random.choice(neighbor_list, self.max_degree, replace=True)
            adj[self.id2idx[nodeid], :] = neighbor_list
        return adj
    ...
    def batch_feed_dict(self, batch_edges):
        batch1 = []
        batch2 = []
        for node1, node2 in batch_edges: # batch_edges[512,2]
